package badentities.kings.utilities;
...
public class Cuboid implements Iterable<Block>, Cloneable, ConfigurationSerializable {
    ...
    public Cuboid(Location l1, Location l2) {
        if (l1 == null || l2 == null) throw new NullArgumentException("Locations cannot be null");
    ...
    public Cuboid contract(CuboidDirection dir) {
        ...
        switch (dir) {
            case Down:
                ...
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, this.x2, (int) face.getUpperY(), this.z2);
            case Up:
                while (face.containsOnly(0) && face.getUpperY() < this.getUpperY()) {
                    face = face.shift(CuboidDirection.Up, 1);
                }
                return new Cuboid(this.worldName, this.x1, (int) face.getLowerY(), this.z1, this.x2, this.y2, this.z2);
            case North:
                while (face.containsOnly(0) && face.getLowerX() > this.getLowerX()) {
                    face = face.shift(CuboidDirection.North, 1);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, (int) face.getUpperX(), this.y2, this.z2);
            case South:
                while (face.containsOnly(0) && face.getUpperX() < this.getUpperX()) {
                    face = face.shift(CuboidDirection.South, 1);
                }
                return new Cuboid(this.worldName, (int) face.getLowerX(), this.y1, this.z1, this.x2, this.y2, this.z2);
            case East:
                while (face.containsOnly(0) && face.getLowerZ() > this.getLowerZ()) {
                    face = face.shift(CuboidDirection.East, 1);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, this.z1, this.x2, this.y2, (int) face.getUpperZ());
            case West:
                while (face.containsOnly(0) && face.getUpperZ() < this.getUpperZ()) {
                    face = face.shift(CuboidDirection.West, 1);
                }
                return new Cuboid(this.worldName, this.x1, this.y1, (int) face.getLowerZ(), this.x2, this.y2, this.z2);
            default:
                throw new IllegalArgumentException("Invalid direction " + dir);
        }
    }
    ...
    public Cuboid getBoundingCuboid(Cuboid other) {
        ...
        int xMin = (int) Math.min(this.getLowerX(), other.getLowerX());
        int yMin = (int) Math.min(this.getLowerY(), other.getLowerY());
        int zMin = (int) Math.min(this.getLowerZ(), other.getLowerZ());
        int xMax = (int) Math.max(this.getUpperX(), other.getUpperX());
        int yMax = (int) Math.max(this.getUpperY(), other.getUpperY());
        int zMax = (int) Math.max(this.getUpperZ(), other.getUpperZ());
